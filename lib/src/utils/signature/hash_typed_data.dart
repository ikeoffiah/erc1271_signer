...
String hashTypedData({
  ...
  final typeStr = '$primaryType(${messageTypes.map((t) => '${t['type']} ${t['name']}').join(',')})';
  final typeHash = keccak256Hex(hex.decode(stringToHex(typeStr).substring(2)));
  final domainTypeStr = 'EIP712Domain(${domainTypes.map((t) => '${t['type']} ${t['name']}').join(',')})';
  final domainTypeHash = keccak256Hex(hex.decode(stringToHex(domainTypeStr).substring(2)));
  ...
  final versionHash = keccak256Hex(hex.decode(stringToHex(version).substring(2)));
  final chainIdEnc = numberToHex(BigInt.from(chainId), size: 32);
  final addressEnc = padHex(verifyingContract.toLowerCase(), size: 32);
  final domainFieldsEncoded = concatHex([nameHash, versionHash, chainIdEnc, addressEnc]);
  final domainEncoded = concatHex([domainTypeHash, domainFieldsEncoded]);
  final hashDomain = keccak256Hex(hex.decode(domainEncoded.startsWith('0x') ? domainEncoded.substring(2) : domainEncoded));
  ...
  final hashStruct = keccak256Hex(hex.decode(messageEncoded.startsWith('0x') ? messageEncoded.substring(2) : messageEncoded));
